...
type WithBufferWorker struct{}

func NewWithBufferWorker() *WithBufferWorker {
	return &WithBufferWorker{}
}

func (w *WithBufferWorker) Run(config *config.Config, msgCount int) {
	...
	msgChan := make(chan *kafka.Message, config.BufferLength)
	go func() {
		defer close(msgChan)
